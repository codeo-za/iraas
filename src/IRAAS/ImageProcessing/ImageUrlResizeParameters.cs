...
public interface IUrlImageResizeParameters : IActiveImageResizeParameters
{
    string Url { get; set; }
}

public interface IActiveImageResizeParameters: IImageResizeParameters
{
    bool OutputFormatSpecified { get; }
    bool PngOutputRequested { get; }
    int? EffectiveWidth { get; }
    int? EffectiveHeight { get; }
    
    void ApplyDefaultsFor(
        string sourceFormat
    );

    void DetermineOutputFormatIfNotSpecified(
        IImageFormat sourceFormat
    );

    Dictionary<string, string> DumpIf(bool enabledGlobally);
}
...
public abstract class ImageResizeParameters : IActiveImageResizeParameters
{
    ...
    protected static readonly Dictionary<string, string> AutoConversionTable = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    protected static readonly Dictionary<PropertyInfo, object> ImageResizeParametersProperties =
        typeof(IImageResizeParameters)
            .GetProperties(BindingFlags.Public | BindingFlags.Instance)
            .ToDictionary(o => o, o => o.PropertyType.DefaultValue());

    protected static readonly Dictionary<string, string> Empty = new();

    protected static readonly Dictionary<PropertyInfo, object> DumpProperties =
